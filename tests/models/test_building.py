import unittest
from prun.models import Building, Planet, BuildingCost
...
class TestBuildingFrom(unittest.TestCase):
    ...
    def test_surface_true(self):
        ...
        result = Building.building_from(self.building, planet)
        self.assertIn(BuildingCost(item_symbol="MCG", amount=400), result.building_costs)

    def test_surface_false(self):
        planet = Planet(surface=False, pressure=1.0, gravity=1.0, temperature=25)
        result = Building.building_from(self.building, planet)
        self.assertIn(BuildingCost(item_symbol="AEF", amount=33.33), result.building_costs)

    def test_low_pressure(self):
        planet = Planet(surface=True, pressure=0.2, gravity=1.0, temperature=25)
        result = Building.building_from(self.building, planet)
        self.assertIn(BuildingCost(item_symbol="SEA", amount=100), result.building_costs)
    ...
    def test_low_temperature(self):
        ...
        result = Building.building_from(self.building, planet)
        self.assertIn(BuildingCost(item_symbol="INS", amount=1000), result.building_costs)
